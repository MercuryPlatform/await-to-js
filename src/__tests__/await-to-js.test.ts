import { to, toAll, promiseVars } from '../await-to-js';
...
describe('promiseVars', async () => {
  ...
  it('should NOT add external properties to the error object', async () => {
    ...
    const [err] = await promiseVars<string, { error: string; extraKey: number }>(promise, {
      extraKey: 1
    });
...
describe('Await toAll test', () => {
  ...
  it('should be able to extend error', async () => {
    ...
    const [error] = await toAll<number, number, CustomError>([promiseOne, promiseThree], {
      ext: 2
    });
